...
if ! (( "$OSTYPE" == "gnu-linux" )); then
  echo "docker-compose-meen-stack-dev runs only on GNU/Linux operating system. Exiting..."
  exit
fi
...
{
    "name": "$PROJECT_NAME",
    "description": "docker-compose-meen-stack-dev project",
    "version": "1.0.0",
    "license": "MIT",
...
#JavaScript
  ...
  if [[ ! -d $PROJECT_NAME ]]; then
    docker-compose run node yarn global add ynpx ember-cli
    docker-compose run node /home/node/.yarn/bin/ember new $PROJECT_NAME --lang en
  fi

  docker-compose up -d
  sleep 5
  docker-compose -f docker-compose.yml up -d
  docker-compose run node sh -c "cd $PROJECT_NAME && yarn start" -d
